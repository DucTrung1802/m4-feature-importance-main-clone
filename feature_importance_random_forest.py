...
if __name__ == "__main__":
    ...
    model_name = 'M4RandomForestSingleOutputRegressor'
    ...
    feature_importance_file = f'{feature_importance_dir}/r_{args.run}_m_{model_name}_f_randomforest.p'
    ...
    if exists(feature_importance_file)==False:        
        ...
        for i_column, column in enumerate(list(Y_train.columns)):
            ...
            l = model.model.estimators_[i_column].feature_importances_
            emb[column] = list(zip(l, used_columns))

        create_directory_if_not_exist(feature_importance_dir)
        pickle.dump(emb, open(feature_importance_file, "wb"))
    else:
        print(f'M4RandomForestSingleOutputRegressor importance already exists')
